...
import modec.metamodel.Classifier;
import modec.metamodel.Instance;
 ...
 * @author Janice Ng
 */
public class SameSuperType extends AbstractPalmBinIntConstraint
	...
	public SameSuperType(IntVar v0, IntVar v1, ScenarioDiagram sd,
			...
			while (iterator0.hasNext()) {
				...
				if (index_e0 > -1) {
					...
					while (iterator1.hasNext() && toBeRemoved) {
						...
						Classifier class_e1 = this.sd
								.getIdxClassifier(index_e1);
						String superClass_e0 = this.getClassSuperType(
								(modec.metamodel.Class) (((ArrayList) (((Instance) (class_e0))
										.getOfClass())).get(0)));
						String superClass_e1 = this.getClassSuperType(
								(modec.metamodel.Class) (((ArrayList) (((Instance) (class_e1))
										.getOfClass())).get(0)));

						if (class_e0 instanceof Instance
								&& class_e1 instanceof Instance
								&& !superClass_e0.equals("")
								&& !superClass_e1.equals("")
			...
			while (iterator1.hasNext() /*& toBeRemoved*/
				...
				if (index_e1 > -1) {
					...
					while (iterator0.hasNext() && toBeRemoved) {
						...
						final int index_e0 = iterator0.next();
						Classifier class_e0 = this.sd
								.getIdxClassifier(index_e0);
						String superClass_e0 = this.getClassSuperType(
								(modec.metamodel.Class) (((ArrayList) (((Instance) (class_e0))
										.getOfClass())).get(0)));
						String superClass_e1 = this.getClassSuperType(
					...
					if (toBeRemoved) {
						...
						((PalmConstraintPlugin) this.getPlugIn())
								.self_explain(expl);
						((PalmIntVar) this.v0).self_explain(PalmIntDomain.DOM,
								expl);
						((PalmIntVar) this.v1).removeVal(index_e1, this.cIdx1,
								expl);
					}
				}
	...
	public String getClassSuperType(modec.metamodel.Class c) {
