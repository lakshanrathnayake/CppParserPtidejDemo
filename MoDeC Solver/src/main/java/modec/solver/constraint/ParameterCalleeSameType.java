...
public class ParameterCalleeSameType extends AbstractPalmBinIntConstraint
	...
	public static void main(final String[] args) {
		...
		final IntVar v_message1 = problem.makeBoundIntVar("msg0", 0,
				nbMessages);
		final IntVar v_message2 = problem.makeBoundIntVar("msg1", 0,
				nbMessages);

		problem.post(new ParameterCalleeSameType(v_message1, v_message2, sd,
				componentsMessages, subclasses));
	...
	public ParameterCalleeSameType(IntVar v0, IntVar v1, ScenarioDiagram sd,
			List componentsMessages, Map subclasses) {
		this.v0 = v0;
		this.v1 = v1;
		this.sd = sd;
				...
				if (index_e0 > -1) {
					...
					while (iterator1.hasNext() && toBeRemoved) {
						...
						if (index_e1 > -1) {
							...
							if (msg1.getDestinationClassifier() instanceof modec.metamodel.Class)
								calleeType = ((modec.metamodel.Class) msg1
										.getDestinationClassifier()).getName();
							else
								calleeType = ((Class) (((ArrayList) (((Instance) msg1
							...
							if (!msg0.isArgumentsEmpty()
								...
								if (msg0.getArguments(0).getType()
										.equals(calleeType.trim())
										&& !msg0.equals(msg1))
									toBeRemoved = false;
								else if (this.subclasses.containsKey(
										msg0.getArguments(0).getType())) {
									List types = (ArrayList) this.subclasses
											.get(msg0.getArguments(0)
													.getType());
					...
					while (iterator0.hasNext()) {
						...
						if (index_e0 > -1) {
							...
							if (!msg0.isArgumentsEmpty()
								...
								String argumentType = msg0.getArguments(0)
										.getType();
								String calleeType = "";
