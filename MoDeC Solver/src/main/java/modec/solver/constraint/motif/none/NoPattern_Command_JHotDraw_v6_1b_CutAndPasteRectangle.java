...
public class NoPattern_Command_JHotDraw_v6_1b_CutAndPasteRectangle {
	public static void main(final String[] args) {
		...
		final IntVar v_aCommand = problem.makeBoundIntVar("aCommand", 0,
				nbClassifiers);
		final IntVar v_anInvoker = problem.makeBoundIntVar("anInvoker", 0,
				nbClassifiers);
		final IntVar v_executeCallee = problem.makeBoundIntVar("executeCallee",
				0, nbClassifiers);
		...
		problem.post(new Caller(v_aClient, v_storeCommand, sd,
				componentsMessages, allClassifiers));
		problem.post(new Callee(v_storeCommand, v_anInvoker, sd,
				componentsMessages, allClassifiers));
		problem.post(new Caller(v_anInvoker, v_execute, sd, componentsMessages,
				allClassifiers));
		problem.post(new Callee(v_execute, v_executeCallee, sd,
				componentsMessages, allClassifiers));
