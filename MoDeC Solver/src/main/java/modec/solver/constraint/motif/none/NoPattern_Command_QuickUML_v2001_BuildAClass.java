...
public class NoPattern_Command_QuickUML_v2001_BuildAClass {
	public static void main(final String[] args) {
		...
		final IntVar v_aReceiver = problem.makeBoundIntVar("aReceiver", 0,
				nbClassifiers);
		final IntVar v_aClient = problem.makeBoundIntVar("aClient", 0,
				nbClassifiers);
		final IntVar v_aCommand = problem.makeBoundIntVar("aCommand", 0,
				nbClassifiers);
		final IntVar v_anInvoker_1 = problem.makeBoundIntVar("anInvoker_1", 0,
				nbClassifiers);
		final IntVar v_anInvoker_2 = problem.makeBoundIntVar("anInvoker_2", 0,
				nbClassifiers);
		//				final IntVar v_executeCallee =
		//				problem.makeBoundIntVar("executeCallee", 0, nbClassifiers);
		final IntVar v_newCommand = problem.makeBoundIntVar("newCommand", 0,
				nbMessages);
		final IntVar v_storeCommand = problem.makeBoundIntVar("storeCommand", 0,
				nbMessages);
		final IntVar v_execute = problem.makeBoundIntVar("execute", 0,
				nbMessages);
		final IntVar v_action = problem.makeBoundIntVar("action", 0,
				nbMessages);
		...
		problem.post(new MessageFollows(v_newCommand, v_storeCommand, sd,
				componentsMessages));
		problem.post(new MessageFollows(v_storeCommand, v_execute, sd,
				componentsMessages));
		//		problem.post(
		//			new MessageFollows(
		//				v_newCommand,
		...
		//				sd,
		//				componentsMessages));
		problem.post(new MessageFollows(v_execute, v_action, sd,
				componentsMessages));

		problem.post(new Created(v_newCommand, v_aCommand, sd,
				componentsMessages, allClassifiers));
		problem.post(new Creator(v_aClient, v_newCommand, sd,
				componentsMessages, allClassifiers));
		problem.post(new Caller(v_aClient, v_storeCommand, sd,
				componentsMessages, allClassifiers));
		problem.post(new Callee(v_storeCommand, v_anInvoker_2, sd,
				componentsMessages, allClassifiers));
		problem.post(new Caller(v_anInvoker_1, v_execute, sd,
				componentsMessages, allClassifiers));
		problem.post(new Callee(v_execute, v_aCommand, sd, componentsMessages,
				allClassifiers));
		problem.post(new Caller(v_aCommand, v_action, sd, componentsMessages,
				allClassifiers));
		problem.post(new Callee(v_action, v_aReceiver, sd, componentsMessages,
				allClassifiers));
		problem.post(new IsContainedInMessage(v_action, v_execute, sd,
				componentsMessages));
		problem.post(new ParameterCalleeSameType(v_storeCommand, v_newCommand,
				sd, componentsMessages, subclasses));
