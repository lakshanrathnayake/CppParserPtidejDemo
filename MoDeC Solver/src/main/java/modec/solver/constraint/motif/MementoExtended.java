	...
	public static void main(final String[] args) {
		...
		final ExecutionTraceParser etp =
				...
				new ExecutionTraceParser(
						"../MoDeC Bytecode Instrumentation Tests/Memento_RunMementoPattern_EVALUATION.trace");
		...
		final IntVar variable5 = problem.makeBoundIntVar("v5", 0,
				nbClassifiers);
		final IntVar variable6 = problem.makeBoundIntVar("v6", 0,
				nbClassifiers);
		final IntVar variable7 = problem.makeBoundIntVar("v7", 0,
				nbClassifiers);

		final Constraint constraintMessageFollowsImmediately1 =
				//			new MessageFollowsImmediately(variable1, variable2, sd);
				new MessageFollows(variable1, variable2, sd,
						componentsMessages);
		final Constraint constraintMessageFollows2 = new MessageFollows(
				variable2, variable3, sd, componentsMessages);
		final Constraint constraintMessageFollowsImmediately3 =
				//new MessageFollowsImmediately(variable3, variable4, sd);
				new MessageFollows(variable3, variable4, sd,
						componentsMessages);

		final Constraint constraintCaller1 = new Caller(variable5, variable1,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCallee1 = new Callee(variable1, variable6,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCreator2 = new Creator(variable6, variable2,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCreated2 = new Created(variable2, variable7,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCaller3 = new Caller(variable5, variable3,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCallee3 = new Callee(variable3, variable6,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCaller4 = new Caller(variable6, variable4,
				sd, componentsMessages, allClassifiers);
		final Constraint constraintCallee4 = new Callee(variable4, variable7,
				sd, componentsMessages, allClassifiers);
		...
		for (int i = 0; i < problem.getPalmSolver().solutions.size(); i++) {
			final PalmSolution solution = (PalmSolution) problem
			...
			System.out.println(sd.getIdxMessage(value0) + " "
					+ sd.getIdxMessage(value1) + " " + sd.getIdxMessage(value2)
