 ...
 * @since  2004/05/16
 */
public class GeneratorIncludingGhosts implements IWalker {
	...
	protected void close(final IFirstClassEntity anEntity) {
		...
		while (itertor.hasNext()) {
			...
			if (!this.listOfKnownEntityNames.contains(name)) {
				this.currentEntity.addUnknownEntity(
						(Entity) this.listOfEntities.get(name));
			}
		}
	...
	private Entity findEntityByType(final IFirstClassEntity anEntity) {
		// Yann 2006/08/11: Member entities...
		// If I cannot find an entity, then it 
		// must be that I am dealing with a
		// member entity.
		String fullyQualifiedName = (String) this.cachedFullyQualifiedNames
				.get(anEntity);
	...
	private Entity findEntityByName(final String anEntityName) {
		...
		//	}
		//	else {
		final Entity entity = (Entity) this.listOfEntities.get(anEntityName);
		return entity;
		//	}
	}
	...
	public void open(final IClass aClass) {
		...
		while (iterator.hasNext()) {
			final Entity entity = this
					.findEntityByType((IFirstClassEntity) iterator.next());
			// Yann 25/11/22: Reduced model
			// After reducing the model, some entity may not exist anymore.
			if (entity != null) {
	...
	protected void open(final IFirstClassEntity anEntity) {
		this.currentEntity = this.findEntityByType(anEntity);
	...
	public void open(final IMethod aMethod) {
		...
		if (!Util.isPrimtiveType(type)) {
			final Entity entity = this.findEntityByName(displayType);
	...
	public void visit(final IAggregation anAggregation) {
		this.currentEntity.addAggregatedEntity(
				this.findEntityByType(anAggregation.getTargetEntity()));
	}

	public void visit(final IAssociation anAssociation) {
		this.currentEntity.addAssociatedEntity(
				this.findEntityByType(anAssociation.getTargetEntity()));
	}

	public void visit(final IComposition aComposition) {
		this.currentEntity.addComposedEntity(
				this.findEntityByType(aComposition.getTargetEntity()));
	}

	public void visit(final IContainerAggregation aContainerAggregation) {
		this.currentEntity.addContainerAggregatedEntity(
				this.findEntityByType(aContainerAggregation.getTargetEntity()));
	}

	public void visit(final IContainerComposition aContainerComposition) {
		this.currentEntity.addContainerComposedEntity(
				this.findEntityByType(aContainerComposition.getTargetEntity()));
	}

	public void visit(final ICreation aCreation) {
		this.currentEntity
				.addCreatedEntity(this.findEntityByType(aCreation.getTargetEntity()));
	}
	...
	public void visit(final IMethodInvocation aMethodInvocation) {
		...
		if (aMethodInvocation.getTargetEntity() != null) {
			this.currentEntity.addKnownEntity(
					this.findEntityByType(aMethodInvocation.getTargetEntity()));
		}
	}
	...
	public void visit(final IUseRelationship aUse) {
		this.currentEntity
				.addKnownEntity(this.findEntityByType(aUse.getTargetEntity()));
	}
}
