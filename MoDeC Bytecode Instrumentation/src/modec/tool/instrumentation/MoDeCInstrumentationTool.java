...
public class MoDeCInstrumentationTool {
	...
	private static final String LOG_TO_FILE_CLASS_NAME = "modec.tool.instrumentation.LogToFile";
	 ...
	 */
	private static void addLogToConstructors(final String logFilename,
			final ClassGen cgen, final Method method) {
		...
		while (i >= 0) {
			if (ihs[i].getInstruction() instanceof ReturnInstruction) {
				final InstructionList ilConstructorExit = logConstructorExit(
						logFilename, MoDeCInstrumentationTool
								.buildMethodSignature(cgen, mgen),
						cgen);
				final InstructionHandle newTarget = il.insert(ihs[i],
						ilConstructorExit);

				final InstructionList ilCalleeID = logCalleeID(logFilename,
						mgen, cgen, false, false);
		...
		while (i < ihs.length && !callThisConstructor) {
			if (ihs[i].getInstruction() instanceof INVOKESPECIAL && ihs[i]
					.getInstruction()
		...
		if (!callThisConstructor) {
			...
			while (i < ihs.length && !callSuperConstructor) {
				if (ihs[i].getInstruction() instanceof INVOKESPECIAL && ihs[i]
						.getInstruction()
						.toString(cgen.getConstantPool().getConstantPool())
						.indexOf(cgen.getSuperclassName().replace('.',
								'/')) > -1) {
		...
		if (callThisConstructor || callSuperConstructor) {
			final InstructionList ilCalleeID = logCalleeID(logFilename, mgen,
					cgen, false, false);
			il.append(ihs[i], ilCalleeID);
			final InstructionList ilConstructorEntry = logConstructorEntry(
					logFilename,
	 ...
	 * @param method the method to be modified
	 */
	private static void addLogToLoops(final String logFilename,
			final ClassGen cgen, final Method method) {
		...
		if (!method.isAbstract()) { // set up the construction tools
			...
			for (int i = ihs.length - 1; i >= 0; i--) {
				...
				if (ihs[i].getInstruction() instanceof GotoInstruction) {
					GotoInstruction gotoInst = (GotoInstruction) ihs[i]
							.getInstruction();
					...
					if (gotoInst.getIndex() < 0) {
						if (!gotoTarget_afterGoto
								.containsKey(gotoInst.getTarget())) {
							countLoops++;
							// to indicate that this target is already 
							// associated to a LOOP ENTRY log
							gotoTarget_afterGoto.put(gotoInst.getTarget(),
									ihs[i + 1]);
							InstructionList instLoopEntry = logLoopEntry(
									logFilename, countLoops + "", cgen);
							oldGotoTarget = gotoInst.getTarget();
							oldTargetIndex = oldGotoTarget.getPosition();
							newGotoTarget = il.insert(oldGotoTarget,
									instLoopEntry);
							gotoInst.updateTarget(oldGotoTarget, newGotoTarget);
							// update every instruction whose target is oldGotoTarget
							InstructionTargeter[] theTargeters = oldGotoTarget
									.getTargeters();
							...
							for (int k = i - 1; k >= 0; k--) {
								if (ihs[k]
										...
										&& ihs[k].getPrev().getPrev()
												.getInstruction()
												.toString(cgen.getConstantPool()
														.getConstantPool())
												.indexOf("ALL LOOPs exit") //.indexOf("ALL LOOPs exit goto")
												== -1) {
									InstructionList newIListReturnInLoop = logAllLoopsExit(
											logFilename, cgen);
									il.insert(ihs[k], newIListReturnInLoop);
								}
							}
						} // insert log LOOP EXIT after gotoInst 
						InstructionList logLoopExitAfterGoto = logLoopExit(
								logFilename, countLoops + "", cgen);
						InstructionHandle loopExitAfterGoto = il
								.append(gotoInst, logLoopExitAfterGoto);
						// update instructions whose target was the instruction
						// AFTER the current GOTO instruction
						InstructionHandle afterGotoInst = (InstructionHandle) gotoTarget_afterGoto
								.get(newGotoTarget);
						InstructionTargeter[] theTargetersAfter = afterGotoInst
								.getTargeters();
						if (theTargetersAfter != null)
							for (int p = 0; p < theTargetersAfter.length; p++)
								theTargetersAfter[p].updateTarget(afterGotoInst,
										loopExitAfterGoto);
						// insert log LOOP EXIT each time 
						// an iteration of a loop ends
						InstructionList logLoopExitBeforeGoto = logLoopExit(
								logFilename, countLoops + "", cgen);
						InstructionHandle loopExitBeforeGoto;
						loopExitBeforeGoto = il.insert(gotoInst,
								logLoopExitBeforeGoto);
						//update instructions whose target was the instruction
						// BEFORE the current GOTO instruction
						InstructionHandle beforeGotoInst = ihs[i].getPrev();
						InstructionTargeter[] theTargetersBefore = beforeGotoInst
								.getTargeters();
						if (theTargetersBefore != null)
							for (int p = 0; p < theTargetersBefore.length; p++)
								theTargetersBefore[p].updateTarget(
										beforeGotoInst, loopExitBeforeGoto);
						int index = i - 1;
						while (oldTargetIndex > -1
								&& ihs[index].getPosition() > oldTargetIndex) {
							InstructionHandle ih_current = ihs[index];
							if (ih_current
									.getInstruction() instanceof GotoInstruction
									&& ((GotoInstruction) ih_current
											.getInstruction()).getTarget()
											.getPosition() > ihs[i]
													.getPosition()) {
								if (ih_current.getPrev().getPrev()
										.getInstruction()
										.toString(cgen.getConstantPool()
												.getConstantPool())
										.indexOf("LOOP exit") == -1) {
									InstructionList newIListExitBeforePositifGoto = logLoopExit(
											logFilename, countLoops + "", cgen);
									il.insert(ih_current,
	 ...
	 * @param method the method to be modified
	 */
	private static void addLogToMethods(final String logFilename,
			final ClassGen cgen, final Method method) {
		...
		il.insert(ihs[0], logMethodEntry(logFilename,
				MoDeCInstrumentationTool.buildMethodSignature(cgen, mg), cgen));
		...
		while (i >= 0) {
			if (ihs[i].getInstruction() instanceof ReturnInstruction) {
				final InstructionHandle newTarget = il
						.insert(ihs[i],
								logMethodExit(logFilename,
										MoDeCInstrumentationTool
												.buildMethodSignature(cgen, mg),
										cgen));

				// main and static methods have no instance CALLER ID
				if (!mg.getName().equals("main") && !mg.isStatic()) {
					final InstructionList ilCallerID = logCalleeID(logFilename,
							mg, cgen, false, false);
	...
	private static String buildMethodSignature(final ClassGen aClassGenerator,
		...
		if (aMethodGenerator.getModifiers() > 0) {
			textToBeLogged
					.append(Modifier.toString(aMethodGenerator.getModifiers()));
		...
		for (int i = 0; i < length; i++) {
			final String argumentType = aMethodGenerator.getArgumentType(i)
					.toString();
	 ...
	 */
	public static void instrumentBytecode(final String logFilename,
			final String originalClasspath, final String[] bytecodeToInstrument,
			final String targetDirectory) {

		try {
			FolderGenerator.copyFolder(originalClasspath, originalClasspath,
					targetDirectory);
			...
			for (int j = 1; j < bytecodeToInstrument.length; j++) {
				...
				if (new File(file).exists()) {
					...
					final JavaClass instrumentedClazz = new ClassParser(
							targetDirectory
									+ clazz.getClassName().replace('.', '/')
									+ ".class")
							.parse();
	 ...
	 */
	private static InstructionList logAllLoopsExit(final String filename,
			final ClassGen cgen) {
		final ConstantPoolGen cp = cgen.getConstantPool();
		...
		newIlist.append(ifact.createInvoke(
				MoDeCInstrumentationTool.LOG_TO_FILE_CLASS_NAME, "allLoopsExit",
	 ...
	 * 						   element of the stacktrace
	 */
	private static InstructionList logCalleeID(final String filename,
			final MethodGen methgen, final ClassGen cgen, final boolean isMain,
		...
		if (!isMain && !isStatic) {
			newIlist.append(InstructionFactory.createLoad(
					...
					new ObjectType("java.lang.Object"), 0));
		}
		else {
		...
		newIlist.append(ifact.createInvoke(
				MoDeCInstrumentationTool.LOG_TO_FILE_CLASS_NAME, "calleeID",
				Type.VOID, new Type[] { new ObjectType("java.lang.String"),
						Type.OBJECT, Type.BOOLEAN, Type.BOOLEAN },
				Constants.INVOKESTATIC));
		return newIlist;
	}
	 ...
	 *						   element of the stacktrace
	 */
	private static InstructionList logConstructorEntry(final String filename,
			final String toBeLogged, final ClassGen cgen) {
	 ...
	 *						   element of the stacktrace
	 */
	private static InstructionList logConstructorExit(final String filename,
			final String toBeLogged, final ClassGen cgen) {
	 ...
	 *						   element of the stacktrace
	 */
	private static InstructionList logLoopEntry(final String filename,
			final String toBeLogged, final ClassGen cgen) {
		...
		newIlist.append(new PUSH(cp, toBeLogged));
		newIlist.append(ifact.createInvoke(
				MoDeCInstrumentationTool.LOG_TO_FILE_CLASS_NAME, "loopEntry",
				Type.VOID,
				new Type[] { new ObjectType("java.lang.String"),
						new ObjectType("java.lang.String") },
				Constants.INVOKESTATIC));
	 ...
	 *						   element of the stacktrace
	 */
	private static InstructionList logLoopExit(final String filename,
			final String toBeLogged, final ClassGen cgen) {
		...
		newIlist.append(new PUSH(cp, toBeLogged));
		newIlist.append(ifact.createInvoke(
				MoDeCInstrumentationTool.LOG_TO_FILE_CLASS_NAME, "loopExit",
				Type.VOID,
				new Type[] { new ObjectType("java.lang.String"),
						new ObjectType("java.lang.String") },
				Constants.INVOKESTATIC));
	 ...
	 *						   element of the stacktrace
	 */
	private static InstructionList logMethodEntry(final String filename,
			final String toBeLogged, final ClassGen cgen) {
		...
		newIlist.append(new PUSH(cp, toBeLogged));
		newIlist.append(ifact.createInvoke(
				MoDeCInstrumentationTool.LOG_TO_FILE_CLASS_NAME, "methodEntry",
				Type.VOID,
				new Type[] { new ObjectType("java.lang.String"),
						new ObjectType("java.lang.String") },
				Constants.INVOKESTATIC));
	 ...
	 *						   element of the stacktrace
	 */
	private static InstructionList logMethodExit(final String filename,
			final String toBeLogged, final ClassGen cgen) {
		...
		newIlist.append(new PUSH(cp, toBeLogged));
		newIlist.append(ifact.createInvoke(
				MoDeCInstrumentationTool.LOG_TO_FILE_CLASS_NAME, "methodExit",
				Type.VOID,
				new Type[] { new ObjectType("java.lang.String"),
						new ObjectType("java.lang.String") },
				Constants.INVOKESTATIC));
